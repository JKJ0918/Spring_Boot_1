...
public class PageResultDTO<DTO, EN> {
    ...
    public PageResultDTO(Page<EN> result, Function<EN,DTO> fn){ // 엔티티를 함수형으로 처리
        //Function<EN, DTO> fn -> Entity 객체드을 dto로 변환해 주는 기능
        // 나중에 어떤 종류의 Page<E> 타입이 생성되더라도, 처리 가능함.
        dtoList = result.stream().map(fn).collect(Collectors.toList());

        // 추가

        totalPage = result.getTotalPages(); // 코배스부교재 p.68 참고 (총 페이지)
    ...
    private void makePageList(Pageable pageable){
        ...
        this.page = pageable.getPageNumber() + 1 ; // 0 부터 시작 함.
        ...
        next = totalPage > tempEnd ; // next 버튼 (다음 페이지 용 코드)
